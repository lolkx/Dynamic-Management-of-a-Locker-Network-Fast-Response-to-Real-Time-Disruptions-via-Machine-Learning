...
labels expected) and data/instances_real_highocc/ (high saturation,
meaningfully positive labels expected) so Option C's model sees the full
risk spectrum. instances_real_highocc/ replaces the earlier synthetic
instances_B/ pool: it is generated by Rudy (2025)'s own, unmodified
Instance::createRandom C++ code (code/rudy_native/instance.h, ported from
nowhere -- literally the same generator that produced data/instances/),
using the same real per-city geography/distance matrices/populations, only
sweeping occupancyProbability higher (0.70-0.93 vs. the published 0.10) and
extending across the same orderFactor range (0.001-0.004) -- see
code/rudy_native/generate_highocc.cpp and the module docstring for the full rationale
...
DATA_DIR_HIGH   = os.path.join(_HERE, '..', 'data', 'instances_real_highocc')
