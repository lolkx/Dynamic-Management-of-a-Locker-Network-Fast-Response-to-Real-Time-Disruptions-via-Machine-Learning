...
distance) changes as p_bias, departure_h, n_iter or alpha_c vary, holding
the others at a baseline.

Model A/B removed (see CLAUDE.md §14): this file used to also sweep beta
for learn_A/B/dynamic_A/B, which no longer exist. What's left:
  ...
  - sweep_alpha_c -- Option C's own ablation (alpha at beta=0 vs beta=baseline),
    self-contained, unaffected by the Model A/B retirement.

Usage
-----
    python run_sensitivity.py <instance.txt>
    python run_sensitivity.py <instance.txt> --sweep p_bias,departure_h
    python run_sensitivity.py <instance.txt> --sweep alpha_c --model-c ../data/models_C/saturation_C_best.pkl

Output
------
    data/sensitivity_p_bias.csv, _departure_h.csv, _n_iter.csv, _alpha_c.csv
...
def _resolve_model_path(explicit: str | None, candidates: list[str], label: str) -> str | None:
    """Explicit --model-c path if given, else the first existing candidate."""
    if explicit:
        return explicit
    for c in candidates:
        if os.path.exists(c):
...
def sweep_p_bias(nodes, dist_matrix, vehicle_cap, depot, locker_cap, orders, dep_h,
                 p_biases: list[float], n_iter: int, verbose: bool = False) -> list[dict]:
...
def sweep_departure_h(nodes, dist_matrix, vehicle_cap, depot, locker_cap, orders,
    ...
    """departure_h only affects 'dynamic' (build_graph_dynamic is loop-level in
    departure_h too, since it's only used inside br_CWS_dynamic's dynamic-time
    gate, not baked into the candidate list). 'standard' has no time
...
def sweep_n_iter(nodes, dist_matrix, vehicle_cap, depot, dep_h,
...
def sweep_alpha_c(nodes, dist_matrix, vehicle_cap, depot, locker_cap, orders, dep_h,
    ...
    alpha AND beta are BOTH loop-level for Option C (unlike beta for the old
...
def main() -> None:
    ...
    p.add_argument('--sweep', default='p_bias,departure_h,n_iter',
                   help='Comma-separated subset of {p_bias,departure_h,n_iter,alpha_c} '
    ...
    p.add_argument('--beta',    type=float, default=BETA_DEFAULT,
                   help='Baseline beta held fixed for alpha_c sweep')
    ...
    p.add_argument('--n_iter',  type=int, default=25,
                   help='GRASP iterations per cell for p_bias/departure_h/alpha_c sweeps '
    ...
    if 'p_bias' in sweeps:
        ...
        rows = sweep_p_bias(nodes, dist_matrix, vehicle_cap, depot, locker_cap, orders,
                            dep_h_baseline, p_biases, args.n_iter, args.verbose)
    ...
    if 'departure_h' in sweeps:
        ...
        rows = sweep_departure_h(nodes, dist_matrix, vehicle_cap, depot, locker_cap, orders,
                                 args.p_bias, departure_hs, args.n_iter, args.verbose)
    ...
    if 'n_iter' in sweeps:
        ...
        rows = sweep_n_iter(nodes, dist_matrix, vehicle_cap, depot,
